          ...
          long long int timeout_when, int *elapsed)
{
    ...
    for (;;) {
        ...
        if (n_pollfds > MAXIMUM_WAIT_OBJECTS) {
            ...
            printf("Cannot handle more than maximum wait objects\n");
        } else if (n_pollfds != 0) {
        ...
        if (deadline <= time_msec()) {
            ...
            printf("wake up from WaitForMultipleObjects after deadline");
...
xgettimeofday(struct timeval *tv)
{
    ...
    if (gettimeofday(tv, NULL) == -1) {
        ...
        printf("gettimeofday failed (%s)", ovs_strerror(errno));
    }
#else
    ULARGE_INTEGER current_time = xgetfiletime();
