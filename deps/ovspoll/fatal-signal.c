...
fatal_signal_init(void)
{
    ...
    if (!inited) {
        ...
        if (!wevent) {
            ...
            printf("Failed to create a event (%s).", msg_buf);
        }
        ...
        for (i = 0; i < ARRAY_SIZE(fatal_signals); i++) {
            ...
            if (old_sa.sa_handler == SIG_DFL
                ...
                printf("signal failed (%s)", ovs_strerror(errno));
            }
#else
...
fatal_signal_unlink_file_now(const char *file)
{
    ...
    if (error) {
        ...
        printf("could not unlink \"%s\" (%s)", file, ovs_strerror(error));
    }
