                   ...
                   const char *condition)
{
    ...
    case 0:
        ...
        printf("%s: assertion %s failed in %s()",
    ...
    for (;;) {
        ...
        if (getcwd(buf, size)) {
            ...
            if (error != ERANGE) {
                ...
                printf("getcwd failed (%s)", ovs_strerror(error));
                return NULL;
            }
            size *= 2;
        }
...
follow_symlinks(const char *filename)
{
    ...
    printf("%s: too many levels of symlinks", filename);
    free(fn);
#endif
...
OVS_CONSTRUCTOR(winsock_start) {
    ...
    if (error != 0) {
        ...
        printf("WSAStartup failed: %s", sock_strerror(sock_errno()));
   }
}
#endif
