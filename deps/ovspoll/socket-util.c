...
set_nonblocking(int fd)
{
    ...
    if (flags != -1) {
        if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) != -1) {
            ...
            printf("fcntl(F_SETFL) failed: %s", ovs_strerror(errno));
            return errno;
        }
    } else {
        //VLOG_ERR("fcntl(F_GETFL) failed: %s", ovs_strerror(errno));
        printf("fcntl(F_GETFL) failed: %s", ovs_strerror(errno));
        return errno;
    }
#else
    unsigned long arg = 1;
    if (ioctlsocket(fd, FIONBIO, &arg)) {
        ...
        printf("set_nonblocking failed: %s", sock_strerror(error));
        return error;
    }
    return 0;
#endif
...
setsockopt_tcp_nodelay(int fd)
{
    ...
    if (retval) {
        ...
        printf("setsockopt(TCP_NODELAY): %s", sock_strerror(retval));
    }
}
...
lookup_ip(const char *host_name, struct in_addr *addr)
{
    if (!ip_parse(host_name, &addr->s_addr)) {
        ...
        printf("\"%s\" is not a valid IP address", host_name);
        return ENOENT;
    }
    return 0;
}
...
lookup_ipv6(const char *host_name, struct in6_addr *addr)
{
    if (!ipv6_parse(host_name, addr)) {
        ...
        printf("\"%s\" is not a valid IPv6 address", host_name);
        return ENOENT;
    }
    return 0;
}
    ...
    if (retval == 1) {
        if (pfd.revents & POLLERR) {
            ...
            if (n < 0) {
                ...
                printf("poll return POLLERR but send succeeded");
                return EPROTO;
            }
        }
        return 0;
        ...
        printf("poll: %s", sock_strerror(sock_errno()));
        return errno;
    } else {
        return EAGAIN;
    }
                          ...
                          const char *s)
{
    ...
    if (port_s && port_s[0]) {
        if (!str_to_int(port_s, 10, &port) || port < 0 || port > 65535) {
            ...
            printf("%s: bad port number \"%s\"", s, port_s);
            goto exit;
        }
    } else {
        port = default_port;
    ...
    if (strchr(host_s, ':')) {
        ...
        if (!ipv6_parse(host_s, &sin6->sin6_addr)) {
            ...
            printf("%s: bad IPv6 address \"%s\"", s, host_s);
            goto exit;
        }
    } else {
        ...
        if (!ip_parse(host_s, &sin->sin_addr.s_addr)) {
            ...
            printf("%s: bad IPv4 address \"%s\"", s, host_s);
            goto exit;
        }
    }
                  ...
                  struct sockaddr_storage *ss)
{
    ...
    if (!host) {
        ...
        printf("%s: host must be specified", target_);
        ok = false;
    } else if (!port && !default_port) {
        //VLOG_ERR("%s: port must be specified", target_);
        printf("%s: port must be specified", target_);
        ok = false;
    } else {
                 ...
                 struct sockaddr_storage *ssp, int *fdp, uint8_t dscp)
{
    ...
    if (fd < 0) {
        ...
        printf("%s: socket: %s", target, sock_strerror(error));
        goto exit;
    }
    ...
    if (error) {
        ...
        printf("%s: set_dscp: %s", target, sock_strerror(error));
        goto exit;
    }
                  ...
                  bool kernel_print_port)
{
    ...
    if (bind(fd, (struct sockaddr *) &ss, ss_length(&ss)) < 0) {
        ...
        printf("%s: bind: %s", target, sock_strerror(error));
        goto error;
    }
    ...
    if (style == SOCK_STREAM && listen(fd, 10) < 0) {
        ...
        printf("%s: listen: %s", target, sock_strerror(error));
        goto error;
    }
    ...
    if (ssp || kernel_chooses_port) {
        ...
        if (getsockname(fd, (struct sockaddr *) &ss, &ss_len) < 0) {
            ...
            printf("%s: getsockname: %s", target, sock_strerror(error));
            goto error;
        }
        if (kernel_chooses_port && kernel_print_port) {
            //VLOG_INFO("%s: listening on port %"PRIu16,
            printf("%s: listening on port %"PRIu16,
    ...
    if (fd >= 0) {
        if (fsync(fd)) {
            if (errno == EINVAL || errno == EROFS) {
                ...
                printf("%s: fsync failed (%s)", dir, ovs_strerror(error));
            }
        }
        close(fd);
    } else {
        ...
        printf("%s: open failed (%s)", dir, ovs_strerror(error));
    }
    free(dir);
#endif
...
xpipe(int fds[2])
{
    if (pipe(fds)) {
        ...
        printf("failed to create pipe (%s)", ovs_strerror(errno));
    }
}
    ...
    *dirfdp = -1;
    ...
    if (strlen(name) > MAX_UN_LEN) {
        ...
        if (error) {
            ...
            printf("Unix socket name %s is longer than maximum "
...
af_inet_ioctl(unsigned long int command, const void *arg)
{
    ...
    if (ovsthread_once_start(&once)) {
        ...
        if (sock < 0) {
            ...
            printf("failed to create inet socket: %s", sock_strerror(error));
            sock = -error;
        }
                    ...
                    const char *cmd_name)
{
    ...
    if (error) {
        ...
        printf("%s: ioctl(%s) failed: %s", name, cmd_name,
                    ovs_strerror(error));
    }
    return error;
}
