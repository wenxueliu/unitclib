...
get_entropy(void *buffer, size_t n)
{
    ...
    if (fd < 0) {
        ...
        printf("%s: open failed (%s)", urandom, ovs_strerror(errno));
    ...
    if (error) {
        ...
        printf("%s: read error (%s)", urandom, ovs_retval_to_string(error));
    }
#else
    int error = 0;
    ...
    if (!CryptGenRandom(crypt_prov, n, buffer)) {
        ...
        printf("CryptGenRandom: read error (%s)", ovs_lasterror_to_string());
        error = EINVAL;
    }
